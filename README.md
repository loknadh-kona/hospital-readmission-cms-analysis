...
![CMS Data](https://img.shields.io/badge/Data-CMS%20HRRP%20FY2026-005EAA)
...
> **Note on the SQL vs. Python overlap:** The four business questions are answered twice by design - in **SQL** (`03_analysis_queries.sql`) as a validation layer proving the dimensional model returns correct aggregates, and in **Python** (`02_statistical_analysis.ipynb`) for deeper statistical testing (t-tests, group comparisons) and visualization.
...
|
|-- sql/
|   |-- 01_create_tables.sql       # DB + staging tables, BULK INSERT raw CMS data
|   |-- 02_data_cleaning.sql       # dim_hospital + fact_readmissions (typed casts, derived flags)
|   |-- 03a_analysis_queries.sql   # 4 business-question queries (SQL validation layer)
|   |-- 03b_fix_null_columns.sql   # Patch: float-to-int cast fix for discharge/readmission counts
|   |-- 03c fix rate scale.sql     # Patch: corrects expected-rate scale (/100) in excess calcs
|   |-- 04_views_for_powerbi.sql   # 4 pre-aggregated views feeding the dashboard
|
|-- python/
|   |-- 01_eda_and_cleaning.ipynb      # Loading, inspection, derived flags
|   |-- 02_statistical_analysis.ipynb  # 4 business questions, t-tests, findings
...
|
|-- data/
|   |-- FY_2026_Hospital_Readmissions_Reduction_Program_Hospital.csv  # Raw CMS HRRP source
|   |-- Hospital_General_Information.csv                              # Raw CMS hospital dimension
|   |-- processed/   # Cleaned/aggregated CSVs generated by the notebooks
|
|-- assets/         # Dashboard screenshots and chart exports
|-- LICENSE
|-- .gitignore
|-- README.md
```
...
sql/03b_fix_null_columns.sql     -- patch: fixes float-to-int casting of counts
sql/03c fix rate scale.sql       -- patch: corrects expected-rate scale (/100)
...
[![LinkedIn](https://img.shields.io/badge/LinkedIn-lvkrishna3-0077B5?logo=linkedin)](https://www.linkedin.com/in/loknadh/)
